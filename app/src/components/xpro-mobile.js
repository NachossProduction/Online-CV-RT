...
export default function XproMobile({currentLang}) {
    return (
        <>
        <section className="lg:hidden w-full h-fit pt-10">
            ...
            <div className="xps-template flex flex-col justify-evenly gap-1 px-4 pb-4">
              ...
              <div className="flex flex-row items-center w-full py-8">
                <div className='w-full flex flex-col justify-evenly gap-3 text-center '>
                  <div className='text-lg'>{currentLang === "EN" ? "Job position: FullStack Developper" : "Poste: Développeur FullStack" }</div>
                  <div className='text-lg'>{currentLang === "EN" ? "For: about 1 year from January 02 2023 to October 01 2023" : "Durée: 1 année, du 02 Janvier 2023 au 01 Octobre 2023"}</div>
                  <div className='text-lg'>{currentLang === "EN" ? "Technical Stack:" : "Stack Technique:" } Svelte + NodeJs + MongoDB </div>
                  <div className='text-lg'>Referent: Florent Michaux - CTO </div>
                </div>
              </div>
            ...
            <div className="xps-template flex flex-col justify-evenly gap-1 px-4 pb-4">
              ...
              <div className="flex flex-row items-center w-full py-8">
                <div className='w-full flex flex-col justify-evenly gap-3 text-center'>
                  <div className='text-lg'>{currentLang === "EN" ? "Job position: Apprentice FrontEnd Developper" : "Poste: Alternant Développeur FrontEnd" }</div>
                  <div className='text-lg'>{currentLang === "EN" ? "For: ~2 years from December 2020 to September 2022" : "Durée: 2 années, de Décembre 2020 à Septembre 2022" }</div>
                  <div className='text-lg'>{currentLang === "EN" ? "Technical Stack:" : "Stack Technique:" } Angular 17 </div>
                  <div className='text-lg'>{currentLang === "EN" ? "Rythm: 3 weeks at work / 1 week in school " : "Rythme: 3 semaines en entreprise / 1 semaine en école (M2I)" } </div>
                  <div className='text-lg'>Referent: Maxime Defenin - Lead Developper </div>
                </div>
              </div>
              ...
              <div className="opinion shadow-light w-full rounded-b-3xl p-4">
                ...
                {currentLang === "EN" ? 
                  ...
                  <div>In fact my referent and i were the only one at the work space <br/> I still remember some rules that he gave me: </div>
                  <div>KISS (Keep it Stupid and Simple) <br/> DRY (Don't Repeat Yourself) <br/> YAGNI (You Ain't Gonna Need It = code only what's needed) </div>
                  </>
                  :
                  <>
                  <div>J'ai aimé la manière dont j'ai été accueilli au sein de la boite. <br/> Malgré la période Covid, j'ai beaucoup appris pendant mes 2 premières semaines d'alternant. </div>
                  <div>Mon référent et moi étaient seuls dans les locaux, les autres en télétravail. <br/> Je me souviens encore des règles de base qu'il m'a donné: </div>
                  <div>KISS (Keep it Stupid and Simple) <br/> DRY (Don't Repeat Yourself) <br/> YAGNI (You Ain't Gonna Need It = code only what's needed) </div>
                  </>
                }
                </div>
